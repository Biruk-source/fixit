...
// FIX: You only need one Supabase client instance
  ...
  Future<List<Worker>> getWorkers({String? location}) async {
    try {
      ...
      // We also only want to show completed profiles on the home screen
      // This is a common requirement to avoid showing half-finished profiles.
      professionalsQuery =
          professionalsQuery.where('profileComplete', isEqualTo: true);
      ...
      // Process each document using the fromJson factory for consistency
      for (var doc in professionalsSnapshot.docs) {
        Map<String, dynamic> data = doc.data() as Map<String, dynamic>;
        data['id'] = doc.id; // Manually add the document ID to the data map

        try {
          // *** THIS IS THE MAIN FIX ***
          // Use the fromJson factory to create the Worker object.
          // This ensures the same data parsing logic is used everywhere.
          final worker = Worker.fromJson(data);
          workers.add(worker);
        } catch (e, s) {
  ...
  Future<Worker?> getWorker(String userId) async {
    try {
      final doc =
          await _firestore.collection('professionals').doc(userId).get();
      if (doc.exists) {
  ...
  Future<String?> uploadProfileVideoToSupabase(
    ...
    try {
      final mimeType = lookupMimeType(platformFile.name,
      ...
      if (kIsWeb) {
        ...
        await _supabaseClient.storage.from(videoBucket).uploadBinary(
            filePath, platformFile.bytes!,
            fileOptions: fileOptions);
      } else {
        if (platformFile.path == null)
          throw Exception("File path is null for mobile upload.");
        await _supabaseClient.storage.from(videoBucket).upload(
            filePath, File(platformFile.path!),
            fileOptions: fileOptions);
      }
  ...
  Future<String?> _uploadFileToSupabase({
    ...
    try {
      ...
      final publicUrl =
          _supabaseClient.storage.from(bucketName).getPublicUrl(pathInBucket);
  ...
  Future<void> declineJobApplication(
      String clientId, String jobId, String workerId) async {
  ...
  Future<void> createNotification({
    ...
    _createNotification(
        userId: userId, title: title, body: body, type: type, data: data);
  }
