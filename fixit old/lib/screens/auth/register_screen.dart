...
import '../../providers/locale_provider.dart'; // Adjust path
...
class _RegisterScreenState extends State<RegisterScreen>
  ...
  // --- UTILITY ---
  bool _isValidEthiopianPhoneNumber(String phoneNumber) {
    final localRegex = RegExp(r'^[79]\d{8}$');
  ...
  String _getPhoneAuthErrorMessage(String code, AppStrings appStrings) {
    switch (code) {
      ...
      /*Use AppString*/ case 'too-many-requests':
        return "Too many attempts.";
      /*Use AppString*/ case 'invalid-verification-code':
        return "Incorrect code.";
      /*Use AppString*/ case 'session-expired':
        return "Code expired. Resend.";
      /*Use AppString*/ default:
  ...
  Future<void> _registerWithEmail() async {
    ...
    try {
      ...
      await _authService.createUserProfile(
          ...
          userType: _userType,
          profession:
              _userType == 'worker' ? _professionController.text.trim() : null,
          photoUrl: null);
      if (!mounted) return;
  ...
  Future<void> _registerWithPhone() async {
    ...
    if (!_isValidEthiopianPhoneNumber(phone)) {
      setState(() => _isLoading = false);
      _showErrorSnackbar("Use 9 digits (e.g. 911...)");
      return;
    }
  ...
  Future<void> _signInWithPhoneCredential(
      PhoneAuthCredential credential, AppStrings appStrings) async {
    if (mounted)
      setState(() {
        _isLoading = true;
  ...
  Future<void> _checkEmailVerifiedStatus(Timer? timer,
      ...
      try {
        await _authService.verifyPhoneNumber(
            ...
            verificationFailed: (e) {
              if (mounted) {
                _showErrorSnackbar(
                    _getPhoneAuthErrorMessage(e.code, appStrings));
                setState(() => _isResending = false);
              }
            },
            codeSent: (id, token) {
    ...
    return Padding(
      ...
      child: LayoutBuilder(builder: (context, constraints) {
        return ToggleButtons(
            ...
            onPressed: (index) {
              if (mounted) {
                setState(() => _selectedMethod = RegistrationMethod
                    .values[index]); /*_formKey.currentState?.reset();*/
              }
            },
  ...
  Widget _buildUserTypeSelector(ThemeData theme, AppStrings appStrings) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Padding(
          ...
          child: Text(appStrings.registerUserTypePrompt,
        ...
        child: Column(key: const ValueKey('registration_form'), children: [
          ...
          _buildInputField(
              ...
              validator: (v) => v == null || v.trim().isEmpty
                  ? appStrings
                      .errorFieldRequired(appStrings.registerFullNameLabel)
                  : null),
            ...
            _buildInputField(
                ...
                validator: (v) {
                  if (v == null || v.trim().isEmpty)
                    return appStrings
                        .errorFieldRequired(appStrings.registerPhoneLabel);
          ...
          if (_selectedMethod == RegistrationMethod.email) ...[
            ...
            _buildInputField(
                appStrings: appStrings,
                label: appStrings.registerPhoneLabel + " (Required)",
  ...
  Widget _buildEmailVerificationContent(
      ThemeData theme, AppStrings appStrings) {
    final userEmail =
    ...
    return Column(
        ...
        children: [
          ...
          const SizedBox(height: 30),
          Text(appStrings.verificationScreenHeader,
          ...
          Padding(
              ...
              child: Text(
                  appStrings.verificationScreenInfo
          ...
          ElevatedButton.icon(
            ...
            onPressed: _isCheckingVerification || _isResending
                ? null
                : () => _checkEmailVerifiedStatus(_emailCheckTimer,
                    isAutoCheck: false),
          ...
          OutlinedButton.icon(
            ...
            onPressed:
                _isResending || _isCheckingVerification ? null : _resendCode,
  ...
  Widget _buildOtpInputContent(ThemeData theme, AppStrings appStrings) {
    ...
    return Column(key: const ValueKey('verification_otp_pending'), children: [
      const SizedBox(height: 20),
      ...
      Padding(
          padding: const EdgeInsets.symmetric(horizontal: 16.0),
          child: Text("Enter the 6-digit code sent to +251$phoneNumber",
              textAlign: TextAlign.center,
              style: theme.textTheme.bodyLarge
                  ?.copyWith(color: theme.colorScheme.onSurfaceVariant))),
      const SizedBox(height: 30),
      ...
      ElevatedButton(
          ...
          /* Use AppStrings */ onPressed:
              _isSubmittingOtp || _isResending ? null : _submitOtp,
          style: ElevatedButton.styleFrom(
              backgroundColor: theme.colorScheme.primary,
              foregroundColor: theme.colorScheme.onPrimary,
              minimumSize: const Size(double.infinity, 50),
      ...
      TextButton(
          child:
              Text(_isResending ? appStrings.resendingButton : "Resend Code"),
          /* Use AppStrings */ onPressed:
              _isResending || _isSubmittingOtp ? null : _resendCode,
    ...
    return [
      IconButton(
          ...
          onPressed: (_isLoading ||
              ...
              : () async {
                  _emailCheckTimer?.cancel();
                  /* <-- FIX: cancel correct timer */ await _authService
                      .signOut();
                  if (mounted)
                    Navigator.of(context)
    ...
    return ElevatedButton(
        ...
        child: _isLoading
            ...
            : Text(
                ...
                /* Use AppStrings */ style: theme.textTheme.labelLarge
                    ?.copyWith(
                        color: theme.colorScheme.onPrimary,
                        fontWeight: FontWeight.bold,
                        letterSpacing: 0.8)));
