// lib/config.dart
class AppConfig {
  // WARNING: Storing secrets directly in code is insecure! Use secure methods.
  static const String basicURL =
      'https://196.188.120.3:38443/apiaccess/payment/gateway'; // SANDBOX URL
  static const String consumerMsisdn =
      "251928211778"; // Your TEST Payer/Payee MSISDN for SANDBOX

  // --- Get these specific values for your SANDBOX account ---
  static const String merchantCode = "251922092601"; // e.g., '76830'
  static const String merchantId =
      "930231098009602"; // e.g., '1403184978483205'
  static const String appid =
      "c4182ef8-9249-458a-985e-06d191f4d505"; // e.g., 'c4182ef8-...'
  static const String appSecret =
      "fad0f06383c6297f545876694b974599"; // e.g., 'fad0f...'
  static const String notifyUrl =
      'https://your-backend.com/notify/telebirr'; // YOUR backend notify URL

  // --- YOUR SANDBOX Private Key ---
  // VERY INSECURE - Load from a secure place
  // MAKE SURE IT HAS PEM MARKERS!

  // ... other config values ...

  // VERY INSECURE - Load from a secure place
  // --- *** MAKE SURE IT LOOKS LIKE THIS *** ---
  static const String privateKey = """-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----""";

  // Placeholder for encrypted PIN - actual encryption needed in service
  static const String consumerPin =
      """""";
}
