...
// import 'package:mime/mime.dart'; // Not needed here now

// --- Models, Services, Screens ---
import '../../models/worker.dart';
import '../../services/firebase_service.dart'; // Correct import
import '../../services/app_string.dart'; // Correct import
// import '../../theme/light_colors.dart'; // Using Theme.of(context) instead
...
class _CreateJobScreenState extends State<CreateJobScreen>
  ...
  List<dynamic> _attachments = []; // Unified list for XFile/PlatformFile
  ...
  Future<void> _loadPreselectedWorker() async {
    ...
    if (mounted)
      setState(() =>
          _isLoading = true); // Show loading specifically for worker fetch
    try {
      final workerData =
  ...
  Future<List<String>> _uploadAttachments() async {
    ...
    try {
      ...
      for (int i = 0; i < _attachments.length; i++) {
        ...
        if (platformFileToUpload != null) {
          ...
          final String? url = await _firebaseService.uploadJobAttachment(
              platformFile: platformFileToUpload, userId: user.uid);
          if (url != null && url.isNotEmpty) {
            ...
            print(
                "    Upload Failed for ${platformFileToUpload.name}. Skipping."); /* Keep item in _attachments */
          }
        }
      } // End for loop
      ...
      if (mounted) {
        setState(() {
          _attachments
              .removeWhere((item) => successfullyUploaded.contains(item));
        });
      }
  ...
  Future<void> _createJob() async {
    ...
    bool requiresSkill = _jobCategories[_selectedCategory!]?.first !=
    ...
    try {
      ...
      if (initialAttachmentCount > 0) {
        ...
        if (_attachments.isNotEmpty && !_isUploading) {
          // Check for remaining files (failures)
          print("Some attachments failed to upload.");
          if (mounted) {
            _showSnackbar(appStrings.createJobSnackbarErrorUploadPartial,
                isError: true);
          ...
          if (mounted) {
            _showSnackbar(appStrings.createJobSnackbarErrorUpload,
                isError: true);
            setState(() => _isLoading = false);
          }
          return;
  ...
  // --- Combined File/Image Picking Logic ---
  Future<void> _pickMedia(String sourceOption) async {
    ...
    try {
      ...
      if (newlyPicked.isNotEmpty) {
        ...
        _showSnackbar(appStrings.createJobSnackbarFileCancelled,
            isError: false);
      }
    } catch (e, s) {
            ...
            child: Padding(
                ...
                child:
                  ...
                  Padding(
                      ...
                      child: Text(appStrings.attachTitle,
                  ...
                  _buildBottomSheetOption(
                      ...
                      'camera'),
                  _buildBottomSheetOption(context, appStrings.attachOptionFile,
                  ...
                  Padding(
                      ...
                      child: TextButton.icon(
                          ...
                          label: Text(appStrings.attachOptionCancel,
  ...
  Widget _buildBottomSheetOption(
      BuildContext context, String title, IconData icon, String sourceOption) {
    final theme = Theme.of(context);
    return ListTile(
  ...
  void _showCalendarDialog() {
    ...
    showDialog(
        ...
        builder: (context) => AlertDialog(
                ...
                title: Text(appStrings.createJobCalendarTitle,
  ...
  Widget build(BuildContext context) {
    ...
    bool skillDropdownEnabled = _selectedCategory != null &&
    ...
    List<String> availableSkills =
        skillDropdownEnabled ? _jobCategories[_selectedCategory]! : [];
                    ...
                    Padding(
                        ...
                        child: FadeInUp(
                            child: _buildSectionCard(
                                title: appStrings
                                    .createJobSelectedWorkerSectionTitle,
                                icon: Icons.person_pin_rounded,
                                colorScheme: colorScheme,
                                children: [
                              _buildThemedWorkerCard(_selectedWorker!, theme)
                            ]))),
                  Padding(
                      padding: const EdgeInsets.only(bottom: 20.0),
                      child: FadeInUp(
                          ...
                          child: _buildSectionCard(
                              ...
                              children: [
                                _buildDropdownFormField(
                                    context: context,
                                    label: appStrings.createJobCategoryLabel,
                                    hint: appStrings.createJobCategoryHint,
                                    value: _selectedCategory,
                                    items: _jobCategories.keys.toList(),
                                    onChanged: (value) {
                                      if (mounted) {
                                        setState(() {
                                    ...
                                    validator: (v) => v == null || v.isEmpty
                                        ? appStrings.createJobErrorCategory
                                        : null,
                                ...
                                _buildDropdownFormField(
                                    ...
                                    validator: (v) => skillDropdownEnabled &&
                                            (v == null || v.isEmpty)
                                ...
                                _buildTextFormField(
                                    ...
                                    validator: (v) {
                                      if (v == null || v.trim().isEmpty)
                                        return appStrings
                                            .createJobDescErrorEmpty;
                                      if (v.trim().length < 20)
                                        return appStrings
                                            .createJobDescErrorShort;
                                      return null;
                                    },
                                    colorScheme: colorScheme,
                                    textTheme: textTheme),
                                ...
                                _buildTextFormField(
                                    ...
                                    validator: (v) {
                                      if (v == null || v.trim().isEmpty)
                                        return appStrings
                                            .createJobBudgetErrorEmpty;
                                      final b = double.tryParse(v.trim());
                                      if (b == null)
                                        return appStrings
                                            .createJobBudgetErrorNaN;
                                      if (b <= 0)
                                        return appStrings
                                            .createJobBudgetErrorPositive;
                                      return null;
                                    },
                                    colorScheme: colorScheme,
                                    textTheme: textTheme),
                                ...
                                _buildTextFormField(
                                    ...
                                    labelText:
                                        appStrings.createJobLocationLabel,
                              ...
                              children: [
                                _buildPickerTile(
                                    title: _selectedDate == null
                                        ? appStrings
                                            .createJobScheduleLabelOptional
                                ...
                                if (_attachments.isNotEmpty)
                                  Padding(
                                      ...
                                      child: _buildAttachmentPreviewGrid(
                                          theme, appStrings)),
                                const SizedBox(height: 16),
          ...
          CircleAvatar(
              ...
              child: worker.profileImage == null || worker.profileImage!.isEmpty
                  ? Icon(Icons.person_outline_rounded,
                      size: 25, color: colorScheme.onSecondaryContainer)
                  : null),
          const SizedBox(width: 12),
          Expanded(
              child: Column(
    ...
    return Container(
        ...
        child: Wrap(
            ...
            children: _attachments.map((attachment) {
              final key = ValueKey(attachment.hashCode); // Use _attachments
              return FadeIn(
                  duration: const Duration(milliseconds: 300),
                  child: _buildAttachmentPreviewItem(
                      key, attachment, theme, appStrings));
            }).toList())); // Use _attachments
  }

  Widget _buildAttachmentPreviewItem(
      Key key, dynamic attachment, ThemeData theme, AppStrings appStrings) {
    final colorScheme = theme.colorScheme;
    bool isImage = false;
    String? extension;
    Uint8List? bytes;
          ...
          Container(
              ...
              child: ClipRRect(
                  ...
                  child: _buildPreviewContent(
                      attachment, isImage, bytes, path, colorScheme))),
          Positioned(
              top: -6,
              right: -6,
              child: Material(
  ...
  Widget _buildPreviewContent(dynamic attachment, bool isImage,
      Uint8List? bytes, String? path, ColorScheme colorScheme) {
    if (isImage) {
  ...
  Widget _buildFileIconPlaceholder(ColorScheme colorScheme) {
    return Center(
        child: Icon(Icons.insert_drive_file_rounded,
            color: colorScheme.primary.withOpacity(0.6), size: 30));
  }
  ...
  Widget _buildUrgentSwitch(ThemeData theme, AppStrings appStrings) {
    ...
    return Row(children: [
      ...
      Expanded(
          child:
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(appStrings.createJobUrgentLabel,
            style: textTheme.bodyLarge?.copyWith(
                fontWeight: FontWeight.w500, color: colorScheme.onSurface)),
        Text(appStrings.createJobUrgentSub,
  ...
  Widget _buildSubmitButton(
      ThemeData theme, AppStrings appStrings, bool isProcessing) {
    final colorScheme = theme.colorScheme;
    final textTheme = theme.textTheme;
    return Container(
        decoration: BoxDecoration(
            gradient: LinearGradient(
                colors: isProcessing
                    ...
                    : [
                        colorScheme.primary,
                        Color.lerp(
                            colorScheme.primary, colorScheme.secondary, 0.6)!
                      ],
