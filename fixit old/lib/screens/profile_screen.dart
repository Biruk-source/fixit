...
import 'package:my_app1/screens/payment/managepayment.dart';
import 'dart:io';
...
import '../theme/light_colors.dart'; // <<< MAKE SURE THIS IS CORRECT
...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  void initState() {
    super.initState();
    // Load profile after the first frame is built to ensure context is available
  ...
  Future<void> _pickAndUploadImage(ImageSource source) async {
    ...
    try {
      ...
      // Ensure the uploading indicator is turned off, even if errors occurred
      // Only set state if it was actually uploading and is no longer loading the profile
      if (mounted && _isUploadingImage && !_isLoading) {
        setState(() {
          _isUploadingImage = false;
        });
      } else if (mounted && _isUploadingImage && _isLoading) {
        // If _loadUserProfile was called in the try block and succeeded,
        // _isLoading will be false, and _isUploadingImage will be false too.
        // If _loadUserProfile failed, _isLoading will be false, but we still need to reset _isUploadingImage.
        // If we are here and _isLoading is still true, it means _loadUserProfile wasn't called or hasn't finished.
        // However, the upload process itself is finished (or failed), so reset the flag.
        setState(() {
          _isUploadingImage = false;
        });
      }
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final strings = AppLocalizations.of(context); // Get localization strings
    final themeProvider = Provider.of<ThemeProvider>(context);

    // Handle case where localization is not yet available
    if (strings == null) {
      return Scaffold(
        appBar: AppBar(title: const Text("Profile")), // Fallback title
        body: Center(
    ...
    final showOverallLoader = _isLoading ||
        (_isUploadingImage &&
            !_isLoading); // Show main loader only when loading profile, not during upload itself if profile is already loaded
    final isDarkMode = themeProvider.themeMode == ThemeMode.dark;

    return Scaffold(
      appBar: AppBar(
        title: Text(strings.appBarMyProfile), // Use localized string
        actions: [
          // Theme Toggle
          IconButton(
            icon: Icon(
              isDarkMode ? Icons.wb_sunny_outlined : Icons.nightlight_round,
              color: theme.colorScheme.onPrimary, // Icon color on AppBar
            ),
            tooltip: isDarkMode
                ? strings.themeTooltipLight
                : strings.themeTooltipDark, // !! Ensure exist !!
            onPressed: () {
          ...
          IconButton(
            ...
            tooltip: strings.languageToggleTooltip, // !! Ensure exists !!
            onPressed: () {
              final currentLocale =
              ...
              // Reload profile after locale change if needed (e.g., if profile has locale-specific data)
              // Or just let the screen rebuild naturally
              WidgetsBinding.instance.addPostFrameCallback((_) {
                if (mounted)
                  _loadUserProfile(); // Refresh profile potentially with new lang data
              });
            },
          ),
          // Logout Button
          IconButton(
            ...
            onPressed:
                showOverallLoader ? null : _signOut, // Disable during load
            tooltip: strings.generalLogout, // Use localized string
          ),
        ],
      ),
      body: showOverallLoader
          ...
          : _userProfile == null
              ? _buildErrorState(context, strings,
                  theme) // Show error state if profile is null after loading attempt
              : _buildProfileContent(
                  context, strings, theme), // Show profile content
    );
  }
    ...
    return RefreshIndicator(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            _buildProfileHeader(context, strings, theme),
            ...
            Divider(color: theme.dividerColor.withOpacity(0.7)),
            const SizedBox(height: 16),
            _buildSettings(context, strings, theme), // Uses navigation now
            const SizedBox(height: 24),
  ...
  Widget _buildProfileHeader(
      BuildContext context, AppStrings strings, ThemeData theme) {
    final profileData = _userProfile!; // Assumed not null here
    final name = profileData.name.isNotEmpty
        ? profileData.name
        : (profileData.email ?? 'No Name');
    final profileImage = profileData.profileImage;

    // !! Ensure 'userRoleUnknown' and getUserTypeDisplayName exist/handle cases !!
    String userTypeDisplay = strings.getUserTypeDisplayName(profileData.role);
    // Fallback if getUserTypeDisplayName doesn't cover the role
    if (userTypeDisplay == profileData.role) {
      userTypeDisplay = profileData.role.isNotEmpty
          ...
          : ('User'); // Use localized unknown role
    }
    ...
    return Card(
      ...
      margin: EdgeInsets.zero, // Use padding of parent instead
          ...
          children: [
            GestureDetector(
              onTap: _isUploadingImage
                  ...
                  : () => _showImagePickerOptions(
                      context), // Disable tap during upload
              child: Tooltip(
                // !! Ensure 'profileEditAvatarHint' exists !!
                  ...
                  children: [
                    CircleAvatar(
                      ...
                      onBackgroundImageError: (exception, stackTrace) {
                        // Log error, maybe show placeholder differently
            ...
            if (userTypeDisplay.isNotEmpty)
              Chip(
                ...
                shape: const StadiumBorder(side: BorderSide.none), // No border
              ),
          ],
        ),
      ),
  ...
  // --- Settings Section Widget (Using Navigator) ---
  Widget _buildSettings(
    ...
    final settingsItems = [
      ...
      },
      {
        ...
        'action': () => navigateToScreen(
            const ProfessionalSetupScreen()), // Navigate to AccountScreen (might be used for client edit profile too)
      },
      {
            ...
            : () {
                ...
                if (isWorker) {
                  ...
                  Navigator.push(
                      ...
                      MaterialPageRoute(
                          builder: (context) =>
                              const ProfessionalSetupScreen()) // Assumes this screen handles both setup and edit
                      ).then((result) {
                    // Refresh profile data when returning from edit screen
                    // The 'result' could potentially carry info if changes were made
                    if (mounted) {
                      print(
                          "Returned from ProfessionalSetupScreen, refreshing profile...");
                      _loadUserProfile();
                    }
                  });
                } else {
                  ...
                  Navigator.push(
                      ...
                      MaterialPageRoute(
                          builder: (context) =>
                              const ProfessionalSetupScreen()) // Replace with your client edit screen
                      ).then((result) {
  ...
  // --- _showSettingsDialog function is removed as it's replaced by _buildSettings using navigation ---

  // !! REMINDER: Add ALL missing strings mentioned in comments (like // !! Ensure '...' exists !!)
  //    to your AppStrings classes (e.g., lib/services/app_strings_en.dart and app_strings_am.dart) !!
}
